...
// Get project members
router.get("/:id/members", protect, checkProjectRole("owner", "editor", "viewer"), getProjectMembers);

// Add or update project member role (Requires owner)
router.post("/:id/members", protect, checkProjectRole("owner"), addProjectMember);

// Remove member from project (Requires owner)
router.delete("/:id/members/:memberId", protect, checkProjectRole("owner"), removeProjectMember);
